...
function getAverage(value1, value2) {
  if (value2 === Number.MAX_VALUE) {
    let a = Number.MAX_VALUE - value1;
    let b = Number.MAX_VALUE - value2;
    console.log(a, b);
    return Number.MAX_VALUE - (a + b) / 2;
  }
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  let lengthA = Math.sqrt(x1 * x1 + y1 * y1);
  let lengthB = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(product / (lengthA * lengthB));
}
...
function roundToPowerOfTen(num, pow) {
  let number = num / Math.pow(10, pow);
