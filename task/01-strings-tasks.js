...
function encodeToRot13(str) {
  ...
  let shifr = "NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm";
  let tmp = "";
  for (let i = 0; i < str.length; i++) {
    let index = original.indexOf(str[i]);
    tmp += index < 0 ? str[i] : shifr[index];
  }
  return tmp;
}
...
function isString(value) {
  ...
  return value.__proto__ == str.__proto__;
}
